...
def burndown_plot(queryset, slug, project_id, cycle_id=None, module_id=None):
    ...
    print("inside burndown plot    ",queryset.total_issues)
    total_issues = queryset.total_issues
    print("in burndown plout s ")
    if cycle_id:
        # Get all dates between the two dates
        print("in cycle id ",cycle_id)
        date_range = [
    ...
    print("dns oanoj tkinv ",completed_issues_distribution)

    for date in date_range:
        cumulative_pending_issues = total_issues
        total_completed = 0
        print("after total comoa d")
        total_completed = sum(
            item["total_completed"]
            for item in completed_issues_distribution
            if item["date"] is not None and item["date"] <= date.date()
        )
        cumulative_pending_issues -= total_completed
        print("is comparable ", date>timezone.now(), "  or s jf ", date<= timezone.now())
