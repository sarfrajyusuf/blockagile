...
export const renderShortDateWithYearAndTimeFormat = (date: string | Date, placeholder?: string) => {
  ...
  const formattedDate = isNaN(date.getTime()) ? placeholder ?? "N/A" : `${month} ${day}, ${year}`;
...
export const renderFormattedPayloadDate = (date: Date | string): string | null => {
  ...
  const formattedDate = format(parsedDate, "yyyy-MM-dd HH:mm:ssXXX");
  return formattedDate;
};
